...
function _get_assembly_election_cons_rhs(jsonUrl,_stFor) {
	...
	_jsonp.send(_jsonAssemblyElectionURL, {
		...
		onSuccess: function (response) {
			...
			if(response!='' || Object.keys(response).length>0) {
				var _consRHSDetail = `<table cellspacing="0" cellpadding="0"><tr><th>${consNameLableLang}</th><th>2023</th><th>2019</th><th>2013</th></tr>`;	
				var _gridWidget = '';
                var _consTop = '';
                let cnameType = 'cname';
				if(langType == 'punjab'){
				...
				Object.keys(response).forEach((key) => {
					...
					_consTop+='<li><a href="'+_rhsConsURLTop+'"><span class="const-cname">'+cname+' | <p class="state_abbr">'+stateAbbr[_stFor]+'</p></span><span class="party-status" style="background: '+_rhsTopConsColor+'"><b>'+ABBR+'</b><em>'+_consStatusVal+'</em></span><span class="last-winner-party">2017: '+WINNER_2018+'</span></a></li>';
