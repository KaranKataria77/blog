		...
		onSuccess: function (response) {
			...
			if(newresObject!='' || Object.keys(newresObject).length>0) {
				...
				Object.keys(newresObject).forEach(function (v,key){ 
					...
					_detailResult = '<div class="brcountday-tallymap tab-content-f alliace-tally-h">'+_asResultListing+ `<p style="font-size: 13px;margin-bottom: 10px;color:#000;padding: 0"><i>*${meghalaya_additional_line} </i></p>`+'</div><div class="brcountday-tallymap tab-content-f hide party-tally-h"> '+_asPartyListing+'</div>';
					_asCnt++;					
				})
