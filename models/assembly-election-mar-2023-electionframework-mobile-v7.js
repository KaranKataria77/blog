		...
		onSuccess: function (response) {
			...
			if(newresObject!='' || Object.keys(newresObject).length>0) {
				...
				Object.keys(newresObject).forEach(function (v,key){ 
					...
					_detailResult = '<div class="brcountday-tallymap tab-content-f alliace-tally-h">'+_asResultListing+ `<p>${meghalaya_additional_line} </p>`+'</div><div class="brcountday-tallymap tab-content-f hide party-tally-h"> '+_asPartyListing+'</div>';
					_asCnt++;					
				})
