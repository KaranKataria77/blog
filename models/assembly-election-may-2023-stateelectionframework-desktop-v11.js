...
console.log("checkkkkkkk liveblog ", apiForLiveblog, url_env, langType);
...
if(hostLocation.indexOf(langType) >= 0){
	...
	}else if(langType == 'karnataka'){
		switch_key = "assembly-elections-2023",
		allianceHead1 = "ವಿಧಾನಸಭಾ ಚುನಾವಣೆ 2023",
	allianceHead2 = "ಸೀಟುಗಳು",
	allianceHead3 = "ಗೆೆಲ್ಲಲು ಸೀಟುಗಳು",
	more_needed = 'Need ',
	more_seats_to_win = "more seats to Win",
	allianceLang = "ಪಕ್ಷ",
	partyLang = "ಪಕ್ಷ",
	winLeadLang = "Wins + Leads",
	strikeRateLang = "ಸ್ಟ್ರೈಕ್ ರೇಟ್*",
...
async function _get_assembly_election_results(jsonUrl,_stFor) {
	...
	// temporary commneting for dry run
	// if(apiForLiveblog != ""){
...
async function _get_assembly_election_key_candidate(jsonUrl,_stFor) {
	let liveBlogForLangUrl = ""
	console.log("checkkk cand_name ", cand_name, cons_name);
	// temporary commenting for dry run
	// if(apiForLiveblog != ""){
	// 	let liveBlogForLangData = await fetch(apiForLiveblog);
	// 		liveBlogForLangData = await liveBlogForLangData.json();
...
//console.log("cons_list_url========>",cons_list_url);
	...
	if(canData) {
		...
		Object.keys(canData).filter(state => (langType == 'bengali' || langType == 'english' || langType == 'punjab') ? true : false).forEach(state => {
