...
async function _get_assembly_election_results_opt(jsonUrl) {
  ...
  let get_opt_json_url = await fetch("https://election.nw18.com/electiondata/electionjson/assembly_election_2022_optimization/live/etv/dynamic/seat_party_summary.json")
        ...
        _asPartyData.forEach(function (value, key) {
          if (key < 8) {
            ...
            var _strikePercentageCheck = isNaN(_strikePercentage)
              ? 0
              : _strikePercentage;
            var _scolor = value.display_name && party_color_code[value.name] ? party_color_code[value.name] : "#1d1d1d";
            // var _scolor = value.color ? value.color : "#1d1d1d";
            _progressBarParty +=
              '<span style="width:' +
              _spercentage +
