...
const party_color_code = {
  ...
  var _glide = "";
  let meghalaya_additional_line = "This line is for testing......."
  function _get_election_json_url_assembly_list() {
    ...
    _jsonp.send(jsonUrl, {
      ...
      onSuccess: function (response) {
        ...
        if (data) {
          var assemblyJsonPath = data.seat_party_summary_all_dynamic;
          ...
          meghalaya_additional_line = data?.tally_note_en
  ...
  async function _get_assembly_election_results_opt(jsonUrl) {
    ...
    var _callbackAssemblyName = "election_assembly_mar2023_all";
  //console.log("_jsonAssemblyElectionURL 12345=====>",_jsonAssemblyElectionURL);
    ...
    let get_opt_json_url = await fetch(_jsonAssemblyElectionURL)
    // let get_opt_json_url = await fetch("https://election.nw18.com/electiondata/electionjson/assembly_election_mar_2023/beta/dynamic/seat_party_summary.json")
    ...
    // onSuccess: function (response) {
      ...
      if (newresObject != "" || Object.keys(newresObject).length > 0) {
        ...
        //   console.log(newresObject,'vnewresObject',v,'newresObjectnewresObject')
          ...
          _detailResult =
            ...
            _asResultListing + `<p style="display: flex;margin-top: 5px;justify-content: flex-end;color:#000"><i>*${meghalaya_additional_line}</i></p>`+
  ...
  async function _get_assembly_election_cons_home_slider(jsonUrl) {
    ...
    // console.log("checkkkkkkkk response ", response);
      ...
      onSuccess: function (response) {
        ...
        if (response != "" || Object.keys(response).length > 0) {
          ...
          Object.keys(response).forEach(function (key) {
            ...
            if (document.querySelector(".as2021-map #myy_" + _consID)) {
              document.querySelector(".as2021-map #myy_" + _consID).style.fill =
                _rhsConsColor;
            }
          });
  ...
  function elecsTab(evt, cityName, id) {
    ...
    showSelectYearMap();
    if (cityName == "TR-MAP" || cityName == "TR") {
      _get_election_json_url_assembly_list_mapclick("tripura");
    } else if (cityName == "ML-MAP" || cityName == "ML") {
  ...
  */
   ...
   let data = await fetch(cons_list_url.replace(/:state:/gi, "gujarat"));
      ...
      data = await fetch(cons_list_url.replace(/:state:/gi, "meghalaya"));
      data = await data.json();
