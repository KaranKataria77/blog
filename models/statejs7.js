...
console.log("checkkkkkk again **************************8");
var _jsonp = (function () {
	let that = {};
...
console.log("checkkkkk ++++++++++++++ ",langType, counting_swither_key);
...
function get_election_json_url_assembly_list() {	
	...
	//console.log("switchUrlAssembly data set======>",switchUrlAssembly);
...
async function _get_assembly_election_results(jsonUrl,_stFor) {
	...
	console.log("checkkkkkkkk name ------------- ", _callbackAssemblyName);
			...
			if(response!='' || Object.keys(response).length>0) {
				...
				//for(var _asState in _asStateArr){
					//if (response.hasOwnProperty(_asState)) {
						console.log("checkkkkkkkk ------------- ", response);
		...
		onSuccess: function (response) {
			...
			if(response!='' || Object.keys(response).length>0) {
				...
				Object.keys(response).forEach((key) => {
					...
					_consTop+='<li><a href="'+_rhsConsURLTop+'"><span class="const-cname">'+cname+' | <p class="state_abbr">'+stateAbbr[_stFor]+'</p></span><span class="party-status" style="background: '+_rhsTopConsColor+'"><b>'+ABBR+'</b><em>'+_consStatusVal+'</em></span><span class="last-winner-party">2017: '+WINNER_2017+'</span></a></li>';
